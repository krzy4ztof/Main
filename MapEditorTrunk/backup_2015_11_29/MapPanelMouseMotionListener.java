	...
	public void mouseMoved(MouseEvent e) {
		...
		if (seg.x != -1) {
			...
			case SELECTION:
				...
				System.out.println(cursorType + " CURSOR NAME");
				...
				/*
				 * if (cursor == CursorChangeState.CHANGE_TO_CROSS) {
				 * mapPanel.getPanel() .setCursor(
				 * Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR)); } else
				 * if (cursor == CursorChangeState.CHANGE_TO_HAND) {
				 * mapPanel.getPanel().setCursor(
				 * Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); } else {
				 * 
				 * }
				 */
				// mapPanel.getPanel().setCursor(

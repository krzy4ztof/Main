...
void Actor::addComponent(shared_ptr<ActorComponent> component) {
	...
	pair<size_t, shared_ptr<ActorComponent>> compPair = make_pair(componentId,
			component);
...
void Actor::tempTestComponents() {
	...
	weak_ptr<TransformComponent> transformComp =
			getComponent<TransformComponent>(
					TransformComponent::COMPONENT_NAME);

	if (shared_ptr<TransformComponent> shrTransformComp =
			transformComp.lock()) {

...
bool ScriptComponent::vInit(const ptree componentNode) {
	optional<const ptree&> optScriptObject = componentNode.get_child_optional(
			SCRIPT_OBJECT_NODE_NAME);
	...
	optional<const ptree&> optScriptData = componentNode.get_child_optional(
			SCRIPT_DATA_NODE_NAME);
...
bool ScriptComponent::readScriptObjectNode(const ptree& scriptObjectNode) {
	...
	optional<string> constrValue = scriptObjectNode.get_optional<string>(
			ATTR_CONSTRUCTOR);
	if (constrValue.is_initialized()) {
		constructor = constrValue.get();
	}

	optional<string> destrValue = scriptObjectNode.get_optional<string>(
			ATTR_DESTRUCTOR);
...
bool ScriptComponent::readScriptDataNode(const ptree& scriptDataNode) {
	...
	optional<string> typeValue = scriptDataNode.get_optional<string>(
			ATTR_ACTOR_TYPE);
...
void ScriptComponent::describeYourself() {
	stringstream ss;
	ss << "ScriptComponent Id: " << vGetId() << "; constructor: " << constructor
			<< "; destructor: " << destructor << "; actorType: " << actorType;
	logger::trace(ss);
}

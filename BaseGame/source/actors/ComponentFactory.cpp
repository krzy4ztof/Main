...
void ComponentFactory::registerComponent(std::string name,
	...
	map<string, ActorComponent* (*)(void)>::iterator findIt =
			creationFunctions.find(name);

	if (findIt == creationFunctions.end()) {
		creationFunctions[name] = factoryFunction; //"zarejestrowano-"+name;
		stringstream ss;
		ss << "zarejestrowano: " << name << "; function: "
				<< creationFunctions[name];
		logger::trace(ss);
...
shared_ptr<ActorComponent> ComponentFactory::create(
		const value_type& componentValueType) {

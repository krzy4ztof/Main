...
void BaseGameLogic::tempTestZipFile(const string& assetsFolder,
		const string& assetsReadMode, const string& assetsSaveMode) {
	...
	string filenameMiniZipOut = assetsFolder + "graphics/tempMiniZip.zip";

	std::vector<std::wstring> paths;
	string pathTemp = assetsFolder + "tempGraphics";
	std::wstring wsTmp(pathTemp.begin(), pathTemp.end());

	paths.push_back(wsTmp);

	zipFile2.test_saveMiniZipCompressedAll(paths, filenameMiniZipOut,
			assetsFolder);

	string filenameAllOut = assetsFolder + "graphics/tempTestAllOut.txt";
	zipFile2.test_saveNotCompressedAll(filenameAllOut, assetsFolder); // in progress
	zipFile2.test_initNotCompressedAll(filenameAllOut); // in progress
	...
	if (myfile.is_open()) {
		...
		if (zipFile.initNotCompressed_222(filename) == false) {
			logger::info("NIE zainicjowano initNotCompressed zip file!!!!");

		} else {

			//zipFile.saveNotCompressed(filenameOut); replaced by zipFile.save

			//zipFile.saveCompressed(filenameZipOut); replaced by zipFile.save

			zipFile.saveNotCompressedAll(filenameOutAll);

			string filenameZipDirHeaderOut = assetsFolder
					+ "graphics/tempTestDirHeaderOut.txt";
			zipFile.test_saveNotCompressedDirFileHeader(
					filenameZipDirHeaderOut);
			zipFile.test_initNotCompressedDirFileHeader(
					filenameZipDirHeaderOut);
			...
			string filenameZipDirHeaderOutZip = assetsFolder
					+ "graphics/tempTestDirHeaderOutZip.txt";

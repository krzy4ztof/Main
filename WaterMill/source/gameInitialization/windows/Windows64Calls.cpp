...
namespace watermill {
	...
	bool isAGameProcessWin64(DWORD processID, const std::string& gameTitle) {
		...
		logger::trace("gameTitle: " + windows64calls::tcharToString(gameTitleTchar) + "; szProcessName: " + windows64calls::tcharToString(szProcessName));
	...
	bool Windows64Calls::checkMemory(const DWORDLONG physicalRAMNeeded, const DWORDLONG virtualRAMNeeded) {
		...
		logger::trace(ss);
		ss << "physical RAM total: " << status.ullTotalPhys << " [byte], " << ramAvailMB << " [MB] " << ramAvailGB << " [GB]";
		logger::trace(ss);
